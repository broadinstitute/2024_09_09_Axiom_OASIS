...
After full verification, compare a deterministic sample with its source TIFFs:

```bash
direnv exec . pixi run -e images python -m image_archive.quality \
  --manifest /absolute/path/images.tsv \
  --output-root /absolute/path/images-jxl \
  --sample-size 64
```
...
## Validated `cpg0037-oasis/axiom` dataset sample

[`cpg0037-oasis-axiom-sample.tsv`](cpg0037-oasis-axiom-sample.tsv) is a deterministic 1,000-row sample of the completed dataset manifest.
Rows were ranked by `SHA-256(UTF-8(source_uri) + NUL + UTF-8(destination_relative))`; the 1,000 lowest hashes were selected and then sorted by `destination_relative`.

[`cpg0037-oasis-axiom-sample-validation.json`](cpg0037-oasis-axiom-sample-validation.json) was generated by the verifier against the external JPEG XL archive on 2026-08-10.
It binds the exact TSV SHA-256 and records `complete=true`, `checked=1000`, and `invalid=0` across 160,998,399 output bytes.
This validates the tracked sample only; it is not a new full-archive validation or a biological-equivalence claim.
